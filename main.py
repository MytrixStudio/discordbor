...
def load_environment_files() -> None:
    """Load local env files without overriding Render-provided variables."""
    candidates = (
...
def get_env(name: str, *aliases: str, required: bool = True) -> str:
    ...
    if required:
        ...
        raise RuntimeError(f"Falta la variable obligatoria {choices}.")
    return ""


def normalize_discord_token(raw_token: str) -> str:
    ...
    placeholder_words = ("pega_aqui", "your_token", "token_nuevo", "replace_me")
    if (
        lower.startswith(("http://", "https://"))
        or "discord.com" in lower
        or "discordapp.com" in lower
        or any(word in lower for word in placeholder_words)
    ):
        raise RuntimeError(
            "DISCORD_TOKEN debe ser el token del bot, no la URL de invitación, "
            "client secret ni un placeholder."
        )
    if any(character.isspace() for character in token) or len(token) < 50:
        raise RuntimeError(
            "DISCORD_TOKEN no parece un token de bot válido. Copia solo el token "
            "desde Discord Developer Portal, sin comillas ni espacios."
        )
    return token


def normalize_bridge_secret(raw_secret: str) -> str:
    secret = raw_secret.strip().strip("\"'").strip()
    if secret.lower().startswith("bearer "):
        secret = secret[7:].strip()
    if len(secret) < 32:
        raise RuntimeError("BRIDGE_SECRET debe tener al menos 32 caracteres.")
    return secret
...
class Config:
    ...
    def load(cls) -> "Config":
        ...
        return cls(
            discord_token=normalize_discord_token(
                get_env("DISCORD_TOKEN", "DISCORD_BOT_TOKEN", "BOT_TOKEN")
            ),
            ...
            guild_id=snowflake("DISCORD_GUILD_ID", "GUILD_ID"),
            panel_channel_id=snowflake("DISCORD_PANEL_CHANNEL_ID", "PANEL_CHANNEL_ID"),
            ...
            bridge_secret=normalize_bridge_secret(
                get_env(
                    "BRIDGE_SECRET",
                    "MYSTRIX_BRIDGE_SECRET",
                    "MINECRAFT_API_SECRET",
                    "WHITELIST_API_SECRET",
                )
            ),
            bridge_server_id=get_env(
                "BRIDGE_SERVER_ID",
                "MYSTRIX_BRIDGE_SERVER_ID",
                required=False,
            ).strip()
